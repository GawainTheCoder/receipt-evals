...
def check_math_error(receipt_details: ReceiptDetails) -> tuple[bool, list[str]]:
    ...
    The summary fields are the trusted reading: subtotal + tax must equal total.
    Item lines are intentionally not used to create math_error, because item-level
    extraction is noisy (duplicated lines, tax and tender lines read as items).
    Every check only runs when all of its inputs are present and parseable, so
    missing values never count as a math error on their own.
    """
...
def deterministic_checks_note(
    *,
    ...
    summary_values = (
        parse_amount(receipt_details.subtotal),
        parse_amount(receipt_details.tax),
        parse_amount(receipt_details.total),
    )
    math_ok_note = (
        " (summary math consistent)"
        if all(value is not None for value in summary_values)
        else " (summary math not checked: missing or unparseable subtotal, tax, or total)"
    )
    math_note = (
        f"math_error={math_error}"
        + (f" ({'; '.join(math_problems)})" if math_problems else math_ok_note)
    )
    warning_note = (
